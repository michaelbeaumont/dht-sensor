...
#[cfg(feature = "async")]
pub mod asynch {
    use super::{DhtError, InputPin, OutputPin, TIMEOUT_US};
    use embedded_hal_async::delay::DelayNs;

    async fn read_bit<P: InputPin>(
        delay: &mut impl DelayNs,
        pin: &mut P,
    ) -> Result<bool, DhtError<P::Error>> {
    ...
    async fn read_byte<P: InputPin>(
        delay: &mut impl DelayNs,
        pin: &mut P,
    ) -> Result<u8, DhtError<P::Error>> {
        let mut byte: u8 = 0;
        for i in 0..8 {
            let bit_mask = 1 << (7 - (i % 8));
    ...
    pub async fn read_raw<P: OutputPin + InputPin>(
        delay: &mut impl DelayNs,
        pin: &mut P,
    ) -> Result<[u8; 4], DhtError<P::Error>> {
        pin.set_high().ok();
    ...
    /// Wait asynchronously until the given function returns true or the timeout is reached.
    async fn wait_until_timeout<E, F>(
        delay: &mut impl DelayNs,
        mut func: F,
    ) -> Result<(), DhtError<E>>
    where
